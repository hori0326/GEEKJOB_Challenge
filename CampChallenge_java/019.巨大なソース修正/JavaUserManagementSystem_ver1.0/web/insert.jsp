    ...
    <body>
        <form action="insertconfirm" method="POST">
            ...
            <select name="year">
                ...
                <option value="<%=i%>" <%if((udb != null) && i == Integer.parseInt(udb.getYear())){ %> 
                        <%= udb.getYear()%> selected <% } %>><%=i%> </option>
                <% } %>
            </select>年
            <select name="month">
                <option value="">--</option>
                <% for (int i = 1; i <= 12; i++) {%>
                <option value="<%=i%>" <%if((udb != null) && i == Integer.parseInt(udb.getMonth())){ %>
                        <%= udb.getYear()%> selected <% } %>><%=i%></option>
                <% } %>
            </select>月
            <select name="day">
                <option value="">--</option>
                <% for (int i = 1; i <= 31; i++) {%>
                <option value="<%=i%>" <%if((udb != null) && i == Integer.parseInt(udb.getDay())){ %> 
                        <%= udb.getYear()%> selected <% } %>><%=i%></option>
                <% }%>
            </select>日
            <br><br>
            ...
            自己紹介文
            <br>
            <textarea name="comment" rows=10 cols=50 style="resize:none" wrap="hard"><% if(udb != null){ %><%= udb.getComment() %><% } %></textarea>
            <br><br>
